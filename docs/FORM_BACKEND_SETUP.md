...
Return to the editor, choose `repairLeadSheet` in the function dropdown, and click **Run** once. (`setupBackend` also runs the same repair.) Google will ask for permission; review it, choose the studio account, open **Advanced**, then choose **Go to Benestudio Form Backend** and **Allow**. Confirm that the spreadsheet now has a `Leads` tab with the headers shown above. If earlier submissions were written before the header row existed, the repair detects the existing `LEAD-...` rows and inserts the missing title row above them without deleting data. The same repair also runs automatically before future submissions. It adds header notes, readable column widths, a filter row, frozen headers, status dropdowns, and date formatting so each lead is easy to scan.
